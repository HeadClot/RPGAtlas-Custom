...
rem
rem  This is a friendly wrapper around a single command:  bun run package:exe
rem  (scripts/package-exe.mjs: Vite build -> Electrobun build -> copy exe).
...
if not exist "package.json" (
  ...
  echo       that holds package.json and the electrobun.config.ts file - then try again.
  goto :fail
)
...
if errorlevel 1 (
  echo.
  echo   [X] "bun install" failed. See the messages above.
  goto :fail
)
echo.
...
echo The first Electrobun build can take several minutes; later builds are much faster.
echo.
call bun run package:exe
if errorlevel 1 (
  echo.
  echo   [X] The build failed. See the messages above.
  echo       A common first-time cause on Windows is a blocked Electrobun CLI
  echo       download. Re-run with network access enabled.
  goto :fail
)
