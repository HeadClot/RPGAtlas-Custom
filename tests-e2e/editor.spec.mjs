...
test.describe("starter packs (phase 6)", () => {
  ...
  test("Village & Countryside Deluxe previews, installs live, and uninstalls cleanly", async ({ page }) => {
    ...
    await page.locator(".ab-railbtn", { hasText: "Packs" }).click();
    const card = page.locator(".ab-pack", { hasText: "Village & Countryside Deluxe" });
    await expect(card).toBeVisible();
    await expect(card.locator(".ab-packpreview")).toBeVisible();
    await expect(card.locator(".ab-meta")).toContainText("installed 0/80");

    await card.locator("button", { hasText: "Install" }).click();
    await expect(card.locator(".ab-meta")).toContainText("installed 80/80", { timeout: 90_000 });

    await page.locator(".ab-railbtn", { hasText: "Tiles" }).click();
    await expect(page.locator(".ab-grid .ab-card")).toHaveCount(80);
    await expect(page.locator(".ab-grid .ab-name", { hasText: "village-summer-grass.terrain" })).toBeVisible();
    await expect(page.locator(".ab-grid .ab-name", { hasText: "village-inn" })).toBeVisible();
    await expect(page.locator(".ab-grid .ab-name", { hasText: "village-gate-ew.pass" })).toBeVisible();
    await expect(page.locator(".ab-cardtags").first()).toContainText("pack:village-countryside-deluxe");
    expect(await page.evaluate(() => window.Assets.tiles.some((tile) => tile && tile.name === "Village Summer Grass"))).toBe(true);
  ...
  test("Dungeon Depths Deluxe previews, installs into the live tile palette, and uninstalls", async ({ page }) => {
    test.setTimeout(90_000);
    await page.goto("/index.html");
    const saveIndicator = page.locator("#save-ind");
    await expect(saveIndicator).toBeVisible();
    ...
    await page.locator(".ab-railbtn", { hasText: "Packs" }).click();
    const card = page.locator(".ab-pack", { hasText: "Dungeon Depths Deluxe" });
    await expect(card).toBeVisible();
    await expect(card.locator(".ab-packpreview")).toBeVisible();
    await expect(card.locator(".ab-meta")).toContainText("installed 0/48");
    ...
    await page.locator(".ab-railbtn", { hasText: "Tiles" }).click();
    await expect(page.locator(".ab-grid .ab-card")).toHaveCount(48);
    await expect(page.locator(".ab-grid .ab-name", { hasText: "dungeon-ancient-flagstone.terrain" })).toBeVisible();
    await expect(page.locator(".ab-grid .ab-name", { hasText: "dungeon-toxic-water" })).toBeVisible();
    await expect(page.locator(".ab-grid .ab-name", { hasText: "dungeon-wall-cross" })).toBeVisible();
    await expect(page.locator(".ab-grid .ab-name", { hasText: "dungeon-chandelier.pass" })).toBeVisible();
    await expect(page.locator(".ab-cardtags").first()).toContainText("pack:dungeon-depths-deluxe");
    expect(await page.evaluate(() => window.Assets.tiles.some((tile) => tile && tile.name === "Dungeon Ancient Flagstone"))).toBe(true);
