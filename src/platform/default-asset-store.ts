/* RPGAtlas — src/platform/default-asset-store.ts
   Picks this session's AssetStore (Phase 6 Stage A): Electrobun FS commands under
...
export async function createDefaultAssetStore(): Promise<AssetStore | null> {
  try {
    ...
    if ((window as any).__ATLAS_ELECTROBUN__) {
      const m = await import("./electrobun/fs-asset-store");
