...
**Status:** COMPLETE (Stages A–E, 2026-07-03). Stage log below, newest first.

Stage E COMPLETE (2026-07-03): starter packs — phase exit. **Registry**:
`img/packs/index.json` (`{packs:[{id,name,desc,license,version,files:[{type,
name,url,kind?,tags?}]}]}`); file URLs resolve against the registry's own
location (bug caught live: root-relative URLs double-pathed → registry-
relative, so remote registries host files beside their index.json); extra
registry URLs are a device setting (`rpgatlas_pack_registries` localStorage,
"+ Add registry URL…" in the tab). **Packs tab** (Asset Browser rail):
cards with license/file-count/installed-n-of-m/size, Install → fetch +
`importAssets` with `pack:<id>` tags (hash-dedupe ⇒ idempotent reinstall +
resumable failures) + live image bind; Uninstall = delete-by-tag with
used-by-project warning counts. **Bundled pack**: "Driftwood Starter" (CC0,
22 files ≈ 1 MB) generated by `scripts/build-starter-pack.mjs` from the
engine's own procedural generators — 10 hue-shifted terrain tiles
(`.pass`/`.terrain` named; via js/assets.js in headless Chromium), 4 chargen
villagers (3×4 sheets), 3 recolored battlers, and Node-synthesized WAVs
(2 chiptune loops w/ loop seams, rain BGS, victory ME, chime SE). Roadmap
deviation noted: no third-party CC0 content ships in-repo — the bundled pack
is self-generated (license-auditable, offline-safe); third-party packs are
registry-ready. Verified live (install 22/22 → tiles/charsets/battlers bound
+ rail counts, uninstall → 0/22, zero console errors; screenshot) + new e2e
(38th spec: install → tag check → uninstall empties the library). Patch
note, wiki (Asset-Browser packs section), `editor.css?v=50`,
`patch-notes.js?v=20` (+shim +help.ts). Full gate green: tsc, eslint,
node --test (16), vitest (186), Playwright **38/38** (goldens byte-stable).
**Phase acceptance:** all five exit criteria hold (imports persist across
reload in IDB and round-trip files with blob-free autosaves; slicer/autotile/
Aseprite flows produce usable assets incl. tag-driven flipbooks; imported
BGM/ambience stream through the deck with procedural audio byte-frozen;
the pack installs/uninstalls idempotently by tag; goldens byte-stable
throughout). Desktop FS store validated by cargo check + the shared
interface tests (no live desktop session this phase). Tagged `phase-6`.
