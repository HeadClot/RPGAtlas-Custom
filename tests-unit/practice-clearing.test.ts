...
describe("Practice Clearing authored combat fixture", () => {
  it("uses database profiles, inherited enemy defaults, and persistent defeat data", () => {
    expect(validateCombatProject(fixture)).toEqual([]);
    expect(resolveActorCombat(fixture, 1)).toMatchObject({ maxHp: 30, profileId: 1 });
    expect(resolveEnemyCombat(fixture, fixture.maps[0].events[0].pages[0])).toMatchObject({ hp: 12, persistentDefeat: true, profileId: 1 });
